  ...
  try {
    if (page && limit) {
      ...
      if (searchParams?.season) {
        whereClause.season = searchParams?.season;
      }
  ...
  try {
    ...
    const products = await db.Product.findAll({
      attributes: [
        ...
        'price',
        'discount',
        // tổng số lượng đã bán trong các đơn SUCCESS; nếu chưa bán thì = 0
        [fn('COALESCE', fn('SUM', col('orderdetails.quantity')), 0), 'totalQuantity'],
      ],
      include: [
        {
          ...
          as: 'orderdetails',
          attributes: [],
          required: false, // LEFT JOIN để sp chưa bán vẫn hiện
          include: [
            {
              model: db.Order,
              as: 'order',
              attributes: [],
              required: false,               // vẫn để LEFT JOIN
              where: { status: 'SUCCESS' },  // điều kiện sẽ vào ON
            },
          ],
        },
      ],
      ...
      limit: 12, // tuỳ UI
      subQuery: false, // tránh subquery không cần thiết
    });

    return { errCode: 0, errMessage: 'OK', DT: products };
  } catch (e) {
    console.error(e);
    return { errCode: -1, errMessage: 'Lỗi máy chủ', DT: e };
  }
};
